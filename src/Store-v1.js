...
function accountReducer(state = intialStateAccount, action) {
  switch (action.type) {
    ...
    case "account/paymentForLoan": {
      if (state.loan === 0) return state;
      return {
        ...state,
        balance: state.balance - action.payload.amount,
        loan: state.load - action.payload.amount,
      };
    }
...
function createCusstomer(fullName, nationalID) {
  return {
...
function updateCusstomerName(fullName) {
  return {
    type: "customer/updateCustomerName",
    payload: { fullName },
  };
}
store.dispatch(createCusstomer("John Doe", "123456789"));
console.log(store.getState());
store.dispatch(updateCusstomerName("Jane Doe"));
console.log(store.getState());
